...
RUN apt-get update && apt-get install -y --no-install-recommends ca-certificates curl postgresql-client tar awscli && rm -rf /var/lib/apt/lists/* \
    ...
    && EXPECTED_SHA256=$(awk -v file="supabase_${SUPABASE_CLI_VERSION}_linux_${TARGETARCH}.tar.gz" '$2 == file { print $1; exit }' /tmp/supabase.checksums) \
    && test -n "$EXPECTED_SHA256" \
    && echo "$EXPECTED_SHA256  /tmp/supabase.tgz" | sha256sum -c - \
