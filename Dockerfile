...
ARG SUPABASE_CLI_VERSION=2.33.9
...
RUN apt-get update && apt-get install -y --no-install-recommends ca-certificates curl postgresql-client tar awscli && rm -rf /var/lib/apt/lists/* \
    && curl -fsSL "https://github.com/supabase/cli/releases/download/v${SUPABASE_CLI_VERSION}/supabase_${SUPABASE_CLI_VERSION}_linux_amd64.tar.gz" -o /tmp/supabase.tgz \
